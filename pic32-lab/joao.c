...
void Keyboard_Configs(void){
    ...
    volatile unsigned int tmp = PORTG; // clear mismatch
    ...
    IPC6bits.CNIP = 2; // set priority and sub-priority
    IPC6bits.CNIS = 0;
    IFS1CLR = _IFS1_CNIF_MASK; // Clears CN interruption flag
    IEC1SET = _IEC1_CNIE_MASK; // Enable CN interruption 
}

void __ISR(_CHANGE_NOTICE_VECTOR, IPL2SOFT) KeyBoardInterrupt(void){
    IEC1CLR = _IEC1_CNIE_MASK; // disables CN interruptions
    ...
    for (unsigned int row = 0; row < 5; row++){ // we need to test all five possibilities
        LATA = (LATA & ~0x1F) | (0x1F & ~scan_position);
        // __delay_ms(1); // TODO: DEBOUNCER
        temp = (~PORTG & 0x000003C0) >> 6; // bitmask 0000_0000_0000_0000_0000_0011_1100_0000
        if (temp){ 
            detected_column = 0;
            while (temp >>= 1){
                detected_column ++;
    ...
    IEC1SET = _IEC1_CNIE_MASK; // enables CN interruptions 
}
